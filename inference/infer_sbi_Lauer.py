...
# x_presimulated = torch.tensor(np.genfromtxt(path+'presimulated_data/'+presim_data,delimiter=',')[:,0:11]).type('torch.FloatTensor')

### simulate ###
theta_presimulated, x_presimulated = simulate_for_sbi(simulator, proposal=prior, num_simulations=100000)
...
for i in range(obs_file.shape[1]):
 ...
 options={'disp': True}).x
    ...
    f.write(EvoModel+',NPE,' + presim_data + ',' + str(ob[i]) + ','.join(str(format(j)) 
                                      for j in
                                      (s_est,
    ...
    # save posterior samples and their log probs
    np.savetxt(path + out + str(ob[i]) + "_NPE_posterior_samples.csv", posterior_samples, delimiter=",")
    np.savetxt(path + out + str(ob[i]) + "_NPE_posterior_logprob.csv", log_probability, delimiter=",")
