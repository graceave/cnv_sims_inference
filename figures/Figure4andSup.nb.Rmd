...
for i in r.obs_names:
        ...
        for j in range(3):
                 x=np.linspace(beta_real*alpha_real/10,beta_real*alpha_real, 100)
...
kl_div.to_csv("SupplementaryTable1.csv")
```
...
plot_dist = function(x, legend=TRUE) {
  ...
  p = ggplot(data = d, aes(x)) +
    ...
    stat_function(fun = function(x) 
           ...
           aes(colour = "True distribution"), size = 1, alpha=1) +
    stat_function(fun = function(x) 
           {dgamma(x, shape = temp$shape_est[1], scale = temp$scale_est[1])},# * 20 * bw}, 
           aes(colour = "Directly inferred MAP distribution"), size = 1, alpha = 0.6) +
    stat_function(fun = function(x) 
    ...
    #####
      stat_function(fun = function(x) 
           ...
           aes(colour = "Distribution from single observation MAPs"), size = 1, alpha = 0.6) +
    stat_function(fun = function(x) 
    ...
    stat_function(fun = function(x) 
           ...
           aes(colour = "Distribution from single observation MAPs"), size = 1, alpha = 0.6) +
    scale_color_manual(breaks = c("True distribution","Directly inferred MAP distribution",
                                  "Distribution from single observation MAPs"), name = "",
                     #values=c("#453781", "#1f968b", "#75d054")) +
                     values=c("black", "blue", "darkgreen")) +
    xlab("selection coefficient") +
    ylab("density") +
    annotate("text",label=as.expression(bquote(atop("E(s)="~.(temp$es[1])~",",delta~"= "~.(temp$mu_real[1])))),
...
a = multi_data %>%
  ...
  scale_x_discrete(name = "",
                   labels=c("0.001_1e-07"  = expression(atop("E(s) = 0.001,", paste(delta, "  = 1e-7"))),
                            "0.001_1e-05" = expression(atop("E(s) = 0.001,", paste(delta, "  = 1e-5"))), 
                            "0.1_1e-07"  = expression(atop("E(s) = 0.1,", paste(delta, "  = 1e-7"))),
                            "0.1_1e-05"  = expression(atop("E(s) = 0.1,", paste(delta, "  = 1e-5"))))) 
...
sup = a/b
sup
ggsave(paste0(fig_path,"SupplementaryFig7.pdf"), plot = sup, width = 20, height = 10, units = "in")
ggsave(paste0(fig_path,"SupplementaryFig7.png"), plot = sup, width = 20, height = 10, units = "in")
```
